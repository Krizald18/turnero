...
<html>
<head>
  <title>Pantalla nacimientos</title>
  <style type="text/css">
    body {
      font-size: 16px;
    }
...
<link
  rel="stylesheet"
  type="text/css"
  href="<?= URL; ?>assets/css/semantic.min.css">
  ...
  app.controller('nacCtrl', async function($scope, $http) {
      ...
      if (turnos.length > 0) {
        new Promise(resolve => {
          if (turnos.length > 0) {
            ...
            u.lang = 'es-MX';
            u.rate = 1.2;
            u.text = `turno ${Number(t.turno)} pase a caja ${Number(t.caja)}`;
            u.onend = resolve;
    ...
    const checar = function() {
      $http({
        ...
        // turnos.filter(
        //  tr => noMostrados.filter(
        //    nm => tr.idturno == nm.idturno
        //  ).length == 0
        // );
